...
# 6. Función Renderizadora Principal (Estructurada para garantizar los 5 gráficos independientes)
def renderizar_dashboard(df, titulo_evento, key_prefix):
    ...
    # ==========================================
    st.subheader(f"📊 1. Episodios Semanales de {titulo_evento}")
    ...
    # ==========================================
    st.subheader(f"📅 2. Episodios Mensualizados de {titulo_evento}")
    ...
    # ==========================================
    # GRÁFICO 3: Tendencia Anual vs Promedios Históricos
    # ==========================================
    st.subheader(f"📉 3. Tendencia Anual vs. Promedios Históricos")
    ...
    if not df_totales_anuales.empty:
        ...
        df_10 = df_totales_anuales[df_totales_anuales["año"].isin(ultimos_10)]
        ...
        fig_hist.add_trace(
            go.Scatter(
                ...
                line=dict(shape="spline", smoothing=1.3, width=4, color="#00CCFF"),
                marker=dict(
                    size=9, color="#FFFFFF", line=dict(width=3, color="#0056B3")
                ),
            )
        )
    ...
    # ==========================================
    # GRÁFICO 4: Comparativo de Últimas Semanas (o distribución complementaria)
    # ==========================================
    st.subheader("📈 4. Comparativo Directo de Últimas Semanas")
    ...
    # ==========================================
    # GRÁFICO 5: Distribución Estratificada por Grupos Etarios (IRAS) con Tabla de Fondo Estilizado
    # ==========================================
    cols_iras = ["ira_m2", "ira_2_11", "ira_1_4a"]
    if all(c in df.columns for c in cols_iras):
        ...
        if not df_et_sum.empty:
            ...
            # Paleta experta: Rojo intenso para máxima alerta clínica (< 2m), Naranja y Azul corporativo
            fig_et = px.bar(
                df_et_sum,
                x="año",
            ...
            # Etiqueta de total general arriba de cada barra apilada
            fig_et.add_trace(
                go.Scatter(
                    x=df_et_sum["año"],
            ...
            # TABLA RESUMEN ESTILIZADA DE ALTO NIVEL
            st.markdown(
                """
